        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {!loading && filtered.length === 0 && (
            <div className="text-center py-12 px-4">
              ...
              <p className="text-xs text-[var(--muted)] mt-1">Book a creator to start a thread</p>
            </div>
          )}
          {filtered.map((t) => {
            ...
            return (
              <button
                key={t.order_id}
                type="button"
                onClick={() => router.push(`/brand/chat?order=${t.order_id}`)}
                className={`w-full flex items-start gap-3 px-4 py-3.5 border-b border-line text-left transition-colors ${
                  isActive ? "bg-[rgba(255,65,24,0.04)] border-l-2 border-l-brand" : "hover:bg-[var(--paper-2)]"
                }`}
              >
      ...
      {activeId ? (
        <div className="flex-1 flex flex-col overflow-hidden">
          <div className="flex items-center gap-3 px-5 py-3 border-b border-line bg-white">
            <button type="button" onClick={() => router.push("/brand/chat")}
            ...
            {activeThread && (
              <>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] font-mono text-[var(--muted)]">{activeThread.order_ref}</p>
                </div>
                <a href={`/brand/orders/${activeThread.order_id}`}
        ...
        <div className="hidden md:flex flex-1 items-center justify-center bg-[var(--paper)]">
          <div className="text-center">
            ...
            <p className="text-xs text-[var(--muted)] mt-1">Pick a thread from the left</p>
          </div>
        </div>
      )}
    </div>
